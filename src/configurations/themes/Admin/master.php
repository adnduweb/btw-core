...
<body x-data="main" hx-ext="morph, ajax-header, head-support, preload" hx-history="false" hx-indicator="#progress" hx-boost="true" class="antialiased relative font-nunito text-sm font-normal overflow-x-hidden collapsible-vertical full <?= detectAgent(); ?>" :class="[$store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ?  'dark' : '', $store.app.menu, $store.app.layout,$store.app.rtlClass]">
    ...
    <div class="main-container text-black dark:text-white-dark min-h-screen transition-fade <?= site_offline() ? 'offline' : '' ?>" :class="[$store.app.navbar]" x-data="{open: false}">
