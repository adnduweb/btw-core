  document.addEventListener("htmx:responseError", function (event) {
    ...
    const htmxErrorModalTitleContent = document.createTextNode(
      "Error: " + xhr.status
    );
    ...
    // Set styles
    htmxErrorModal.setAttribute(
      "style",
      "position: absolute; max-width: 90%; left: 50%; transform: translateX(-50%); z-index: 99999; background: #fbe0e0; padding: 20px; border-radius: 5px; font-family: sans-serif; top: 50px;"
    );
    htmxErrorModalTitle.setAttribute("style", "display: inline-block;");
    htmxErrorModalCloseButton.setAttribute(
      "style",
      "border: 1px solid; padding: 5px 8px 3px 8px; display: inline-block; float: right;"
    );
    htmxErrorModalContent.setAttribute(
      "style",
      "border: 1px solid #ccc; width: 80vw; height: 80vh"
    );
