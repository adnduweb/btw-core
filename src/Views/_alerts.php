<?php foreach ($alerts as $alert) : ?>
    ...
    <div x-data x-init="$store.toasts.createToast('<?= $content ?>.', '<?= $class; ?>')" class="bg-gray-700 border-t-4 border-blue-600 text-white p-3 hidden"></div>
<?php endforeach; ?>


<div x-data class="fixed inset-x-0 mx-auto bottom-5 overflow-hidden max-w-xl sm:w-full space-y-5 z-50">
    <template x-for="(toast, index) in $store.toasts.list" :key="toast.id">
        <div x-show="toast.visible" @click="$store.toasts.destroyToast(index)" x-transition:enter="transition ease-in duration-200" x-transition:enter-start="transform opacity-0 translate-y-2" x-transition:enter-end="transform opacity-100" x-transition:leave="transition ease-out duration-500" x-transition:leave-start="transform translate-x-0 opacity-100" x-transition:leave-end="transform translate-x-full opacity-0" class="bg-gray-900 bg-gradient-to-r text-white p-3 rounded mb-3 shadow-lg flex items-center" :class="{
				'from-blue-500 to-blue-600': toast.type === 'info',
				'from-green-500 to-green-600': toast.type === 'primary',
				'from-yellow-400 to-yellow-500': toast.type === 'warning',
				'from-red-500 to-pink-500': toast.type === 'danger',
				}">
