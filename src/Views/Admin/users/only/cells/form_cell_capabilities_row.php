<tr class="relative dark:hover:bg-gray-800 hover:bg-slate-50 list-group-item">
    <td class="border-dashed border-t border-gray-300 text-gray-700 px-6 py-3 dark:text-gray-200">
        <label class="form-check  form-check-sm form-check-custom form-check-solid <?= ($user->hasPermission($rowPermission[0])) ? 'checkbox-success' : '' ?>">
            ...
            <?= $user->can($rowPermission[0]) ? 'in-group' : '' ?>" 
            type="checkbox" name="permissions[]" value="<?= $rowPermission[0] ?>" 
            <?php if ($user->hasPermission($rowPermission[0]) || isset($permissionsMatrix[$rowPermission[0]])) : ?> checked="checked" <?php endif ?> 
            <?= ($user->inGroup('superadmin')) ? 'disabled checked="checked"' : '' ; ?> 
            <?= (!$user->inGroup('superadmin') && !auth()->user()->inGroup('superadmin')) ? 'disabled' : '' ; ?> 
            <?= isset($permissionsMatrix[$rowPermission[0]]) ? 'disabled' : '' ?> 
            hx-put="<?= route_to('users-permissions-toggle-only', $user->id, $rowPermission[0]); ?>" 
