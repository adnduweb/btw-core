<?= form_open(route_to('note-edit', $noteModal->getIdentifier()), [
    'id' => 'kt_notes_form_create',
    'hx-post' => route_to('note-edit', $noteModal->getIdentifier()),
    'hx-target' => '#addnote',
...
<div class="px-4 py-3 text-right sm:px-6 bg-gray-100 rounded-b-md">
    ...
    <button type="button" class="inline-flex justify-center cursor-pointer bg-gray-500 text-white active:bg-gray-600 dark:bg-gray-900  font-bold px-4 py-2 text-sm rounded-md shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-in-out duration-1200 transition-all" @click="showNoteEditModal = false">
        <?= lang('Btw.close'); ?>
    </button>
</div>
<?= form_close(); ?>