    ...
    <div class="flex flex-col space-y-5">
        <?php if (!empty($allBrowser)): ?>
            <?php foreach ($allBrowser as $key => $value): ?>
                ...
                <?php if (isset(config('Visits')->browsers[strtolower($key)])): ?>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="px-3 flex-initial w-full">
                            <div class="w-summary-info flex justify-between font-semibold text-white-dark mb-1">
                                ...
                                <p class="ltr:ml-auto rtl:mr-auto text-xs">
                                    <?= count($value) ?>
                                </p>
                            </div>
                            <div>
                                <div
                                    ...
                                    <div class="<?= config('Visits')->browsers[strtolower($key)]['class']; ?> w-full h-full rounded-full relative before:absolute before:inset-y-0 ltr:before:right-0.5 rtl:before:left-0.5 before:bg-white before:w-2 before:h-2 before:rounded-full before:m-auto"
                                        style="width: <?= count($value) ?>%;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
