<div class="mb-4 flex flex-col md:flex-row justify-between items-center">
    ...
    <div class="card-toolbar-only">
        ...
        <div class="flex flex-row-reverse" data-kt-datatable-toolbar="only" hx-boost="true">
            ...
            <?php if (!empty($add)) : ?>
                <a href="<?= $add['href']; ?>" <?php if (!empty($add['htmxFormat'])) : ?> <?=  $add['htmxFormat']; ?> <?php endif; ?> class="ml-2 inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-medium text-white hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 active:bg-blue-600 transition sm:text-sm">
                    <?= $add['titre']; ?>
                </a>
            <?php endif; ?>
