<div x-data="{ <?= $params['type']; ?>: false  }" x-show="<?= $params['type']; ?>" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div id="modal" x-show="<?= $params['type']; ?>" x-on:closemodal.window="<?= $params['type']; ?> = false" x-on:openmodal<?= $params['name']; ?>.window="<?= $params['type']; ?> = true" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" hx-target="this">
        <div id="dialog" x-show="<?= $params['type']; ?>" class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
            ...
            <div x-cloak x-show="<?= $params['type']; ?>" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-xl my-20 text-left transition-all transform bg-white dark:bg-gray-800 rounded-lg shadow-xl 2xl:max-w-2xl" x-on:click.outside="$el.classList.add('animate-buzz'); setTimeout(() => $el.classList.remove('animate-buzz'), 500)" data-loading-states>
                ...
                <div id="<?= $params['identifier']; ?>" hx-trigger="load">
                    <?= $params['view']; ?>
                </div>
