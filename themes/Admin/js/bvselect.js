...
export const SettingBVSelect = () => {
  class BVSelect {
    constructor({
      selector =  document.querySelector(".defaultId"),
      width = "100%",
      searchbox = false,
      ...
      this.SetupListOptions = function () {
        ...
        for (var i = 0; i < x.length; i++) {
          ...
          if (optionImg) {
            ...
            if (optionIcon) {
              var has_attachment =
                "<i class='" + optionIcon + "' aria-hidden='true'></i>";
            } else {
              var has_attachment = "";
            }
          }
        ...
        document.querySelectorAll("#ul_" + randomID + " li").forEach((item) => {
          item.addEventListener("click", (e) => {
            ...
            if (
              item.classList.contains("bv_disabled") ||
              item.classList.contains("nofocus") ||
              item.classList.contains("bv_separator")
            ) {
            } else {
              if (selected_option.attributes.multiple) {
                ...
                if (selectedMultiple.length == 0) {
                  ...
                  for (var i = 0; i < selectedMultiple.length; i++) {
                    ...
                    SelectedNames +=
                      ", " + selected_option[indexValFromArray - 1].innerHTML;
                  }
                ...
                document.getElementById("main_" + randomID).innerHTML =
                  ...
                  "<i id='arrow_" +
                  randomID +
                  "' class='arrows_bv arrow down'></i>";
              } else {

                // Get Index of option
                (this.selector)
                  .getElementsByTagName("option")[index + numberless].selected =
                  "selected";
                ...
                (this.selector).value = item.getAttribute('data-option-value');
                ...
                if ("createEvent" in document) {
                    ...
                    (this.selector).fireEvent("onchange");
                }
                ...
                if (
                  document.body.contains(document.getElementById("deletebg"))
                ) {
              ...
              if (this.searchbox == true) {
                ...
                Array.from(
                  document.querySelectorAll("#ul_" + randomID + " li")
          ...
          if((this.selector).options[(this.selector).selectedIndex].text != ""){
            document.getElementById("main_" + randomID).innerHTML =
            (this.selector).options[(this.selector).selectedIndex].text +
            "<i id='arrow_" +
            randomID +
            "' class='arrows_bv arrow down'></i>";
          }
      ...
      this.CriarDivBase = function () {
		...
		var bv = (this.selector).nextSibling;
        if(bv.classList != undefined){
           return false;
        }
          ...
          .insertAdjacentHTML(
            ...
            '<div id="bv_' +
              ...
              '" data-search="' +
              this.searchbox +
              '" class="bv_mainselect"></div>'
          );
        document
          .getElementById("bv_" + randomID)
          .insertAdjacentHTML(
            "afterbegin",
            '<div id="main_' +
              randomID +
              '" style="width:' +
              this.width +
              ';" class="bv_atual bv_background"></div><ul id="ul_' +
              randomID +
              '" class="bv_ul_inner bv_background"></ul>'
          );
        ...
        if (this.searchbox == true) {
          document
            ...
            .insertAdjacentHTML(
              "afterbegin",
              '<li class="nofocus"><div class="innerinput"><input placeholder="' +
                SearchPlaceholder +
                '" class="bv_input appearance-none block px-4 py-3 w-full rounded-md bg-gray-100 border-gray-100 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm leading-tight focus:outline-none dark:text-gray-200 dark:bg-gray-900" id="input_' +
                randomID +
                '" type="text"></div</li>'
            );
        }
        ...
        if (!selected_option.attributes.multiple) {
          ...
          if (FoundSelectedOP == 1) {
            first_option_text =
              selected_option.options[selected_option.selectedIndex].text;
          } else {
        ...
        document.getElementById("main_" + randomID).addEventListener(
          ...
          function () {
            ...
            if (window.innerWidth < breakpoint) {
              document
                .getElementById("ul_" + randomID)
                .classList.add("bv_ul_mobile");
              MDivSelector
                .insertAdjacentHTML(
                  "afterend",
                  '<div id="deletebg" class="bv_ul_bg"></div>'
                );
              document.body.classList.add("stop-scrolling");
            } else {
              document
                .getElementById("ul_" + randomID)
                .classList.remove("bv_ul_mobile");
            }
            ...
            if (
              ...
              Array.from(document.querySelectorAll(".arrows_bv")).forEach(
                function (val) {
              ...
              Array.from(document.querySelectorAll(".bv_ul_inner")).forEach(
                function (val) {
      ...
      function FixVerticalViewPort() {
        ...
        if (
          Math.round(currentElementView + 50) > Math.round(currentWindowView)
        ) {
      ...
      if (this.searchbox == true) {
        ...
        document.getElementById("input_" + randomID).addEventListener(
          ...
          function () {
            ...
            Array.from(document.querySelectorAll(".bv_separator")).forEach(
              function (val) {
                val.style.display = "none";
              }
            );
            ...
            for (i = 0; i < li.length; i++) {
              ...
              if (filter == "") {
                ...
                if (
                  ...
                  !li[i].classList.contains("bv_separator")
                ) {
                  li[i].style.display = "";
                } else {
      ...
      document.addEventListener(
        ...
        function (e) {
          if (
            !e.target.closest(".bv_ul_inner") ||
            !event.target.classList.contains("bv_input")
          ) {
            if (
              !event.target.classList.contains("bv_input") &&
              !event.target.classList.contains("bv_atual")
            ) {
    ...
    SetOption(properties) {
      var selectorIndex = (this.selector);
...
export default SettingBVSelect;
